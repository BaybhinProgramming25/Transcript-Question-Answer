from helpers.constants import AVOIDED_CLASSES, GRADES_VALUE_MAPPING, TRANSFER_FORMARTS, MATH_PLACEMENT_MAPPING, TERMINATING_WORDS
from helpers.buildcourse import create_course_string

__all__ = ['get_content_from_each_sem']
...
def get_content_from_each_sem(sm_ll, pdf_pages_list):
    ...
    student_id = _get_student_id(pdf_pages_list[slow_ptr.data[1]])
    ...
    while fast_ptr:
        ...
        course_names = _get_course_name(sub_arr)
        course_numbers = _get_course_number(sub_arr)
        course_descriptions = _get_course_description(sub_arr)
        course_attempted_points = _get_course_points_attempted(sub_arr)
        course_earned_points = _get_course_points_earned(sub_arr)
        course_letter_grades = _get_course_letter(sub_arr)
        course_total_points = _get_course_total_points(sub_arr)
...
def _get_student_id(text):

    student_id = text.split(":")[-1].strip()
    return student_id

def _get_course_name(sub_arr):
...
def _get_course_number(sub_arr):

    course_numbers = []
    for value in sub_arr:
        value = value.strip()
        if _is_terminate(value):
            break
        if _is_valid_course_format(value):
            course_numbers.append(value)
    return course_numbers


def _get_course_description(sub_arr):
...
def _get_course_points_attempted(sub_arr):
...
def _get_course_points_earned(sub_arr):
    ...
    for i in range(len(sub_arr)):
        ...
        if _is_terminate(value):
            break
        if _is_valid_course_format(value) and points_earned_found:
            points_earned_found = False
            ...
            if parts[0].isdigit() and parts[1].isdigit():
                if i + 1 < len(sub_arr):
                    ...
                    if len(next_value) > 1 and next_value[0].isdigit() and next_value[1].isdigit():
                        ...
                        points_earned_container.append("") # Empty string as placeholder

    return points_earned_container

def _get_course_letter(sub_arr):
    ...
    for i in range(len(sub_arr)):
        ...
        if _is_terminate(value):
            break
        if _is_valid_course_format(value) and letter_found:
            letter_found = False
...
def _get_course_total_points(sub_arr):
    ...
    for i in range(len(sub_arr)):
        ...
        if _is_terminate(value):
            break
        if  _is_valid_course_format(value) and total_points_found:
            total_points_found = False
